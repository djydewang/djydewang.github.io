---
layout: post
title:  "postgresql之FSM"
date:   2019-05-15 21:15:10
...
```
该结构定义在普通postgres page的基础之上，可以看出其结构十分精简，fp_next_slot用来提高多后端进程访问FSM的并发性，尽量将他们要访问的可用page错开。
fp_nodes用来存储二叉树，非叶子节点n的左孩子和右孩子分别为2n+1和2n+2。利用一个字节来表示page的剩余空间，主要是为了精简结构，使得查找速度极快。
且利用一个字节的表示page的剩余空间的精准度是能接受的,一个字节能表示1-255，各个数值所表达的剩余空间如表所示
```
 * Range	 Category
 * 0	- 31   0
 * 32	- 63   1
 ...
 * 8164 - 8192 255
```
精确的可用空间已经被记录在了每一个page头中。
实际运用中一个FSM page可以管理多少page呢？
```
/*
...
```
BLCKSZ一般为8kB, 这里的SizeOfPageHeaderData指普通page的头，一般为24字节，按照上述定义可以计算得叶子节点数量为`（8192-24-1-（8192/2 - 1）= 4072`
FSM最多需要管理`2^32-1` pages,因此仅靠一页FSM显然是不够的因此引入多层级的FSM，即非0级的FSM的叶子节点为其下级FSM page的根节点。
如此一来，一个三层结构的FSM,每个FSM page仅需提供1626(1626^3刚好大于2^32 -1)个叶子节点即可管理2^32-1 pages。而postgres规定BLCKSZ必须为2的幂次,
因此对于一个三层结构的FMS，其需要的BLCKSZ至少为4096`((4096 - 24 - 1 - (4096 / 2 - 1)) = 2024 > 1626)`.
同理对于一个四层结构的FMS, BLCKSZ的大小至少为512。
```
/*
